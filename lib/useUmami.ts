...
export const useUmami = () => {
  ...
  const trackCustom = useCallback((
    ...
    sendUmamiEvent(eventName, parameters);
  }, []);
 ...
 */
export const useUmamiButton = (category: string) => {
 ...
 */
export const useUmamiForm = (formName: string) => {
  const { trackInteraction } = useUmami();
 ...
 */
export const useUmamiAI = (featureName: string) => {
  const { trackAI } = useUmami();
